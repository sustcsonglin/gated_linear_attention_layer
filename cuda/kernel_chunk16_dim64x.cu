...
__global__ void fwd_o_chunk16_dim64x(int batchSize, int M, int N_K, int N_V,
  ...
  for(int gg =0; gg < num_loop; gg+=1){
        ...
        FLOAT4(V_tile[B_row][B_column]) = FLOAT4(V[B_row * N_V + B_column]);        
        float4 tmp = FLOAT4(G_V[B_row * N_V + B_column]);
        ...
        FLOAT4(O[B_row * N_V + B_column]) = FLOAT4(threadResults[0]);                
        V += 64;
        G_V += 64;
        O += 64; 
        __syncthreads();
...
__global__ void compute_d_kernel(int batchSize, int M, int N_K, int N_V,
    ...
    for(int gg =0; gg < N_V; gg+=64){
        float threadResults_dV[4] = {0.0};
        float threadResults_dgv[4] = {0.0};        
        
        FLOAT4(V_tile[B_row][B_column]) = FLOAT4(V[B_row * N_V + B_column]);
        FLOAT4(GV_tile[B_row][B_column]) = FLOAT4(GV[B_row * N_V + B_column]);
        FLOAT4(DO_tile[B_row][B_column]) = FLOAT4(DO[B_row * N_V + B_column]);
        __syncthreads();
        ...
        FLOAT4(DV[B_row * N_V + B_column]) = FLOAT4(threadResults_dV[0]);        
        FLOAT4(DG_V[B_row * N_V + B_column]) = FLOAT4(threadResults_dgv[0]);        
        __syncthreads();
    ...
    for(int gg =0; gg < N_K; gg+=64){
        float threadResults_dQ[4] = {0.0};
        float threadResults_dK[4] = {0.0};        
        float threadResults_dgk[4] = {0.0};        

        FLOAT4(GK_tile[B_row][B_column]) = FLOAT4(GK[B_row * N_K + B_column]);
        FLOAT4(Q_tile[B_row][B_column]) = FLOAT4(Q[B_row * N_K + B_column]);
        FLOAT4(K_tile[B_row][B_column]) = FLOAT4(K[B_row * N_K + B_column]);
        ...
        FLOAT4(DQ[B_row * N_K + B_column]) = FLOAT4(threadResults_dQ[0]);                
        FLOAT4(DK[B_row * N_K + B_column]) = FLOAT4(threadResults_dK[0]);  
        FLOAT4(DG_K[B_row * N_K + B_column]) = FLOAT4(threadResults_dgk[0]);
