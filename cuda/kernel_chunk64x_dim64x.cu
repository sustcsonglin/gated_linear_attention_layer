...
__global__ void __launch_bounds__(K9_NUM_THREADS)
    fwd_attn_chunk128(int M, int N, int K,  float *A, float *B,
                   float *G, float *C) {
    ...
    for (uint wnIdx = 0; wnIdx < WNITER; ++wnIdx) {
      ...
      for (uint resIdxM = 0; resIdxM < TM; resIdxM += 1) {
        for (uint resIdxN = 0; resIdxN < TN; resIdxN += 4) {
          ...
          // // load C vector into registers
          // float4 tmp = reinterpret_cast<float4 *>(
          //     &O_interim[(threadRow * TM + resIdxM) * N + threadCol * TN +
          //                resIdxN])[0];
          // // // perform GEMM update in reg
          ...
          // tmp.x = tmp.x * tmp2.x + regDGN[i + 0];
          // tmp.y = tmp.y * tmp2.y + regDGN[i + 1];
          // tmp.z = tmp.z * tmp2.z + regDGN[i + 2];
          // tmp.w = tmp.w * tmp2.w + regDGN[i + 3];
