...
g_A = torch.randn(batch, num_head, num_chunk, chunk_size, d_head, device='cuda').uniform_(0.8, 0.99).log()
g_A_cumsum = g_A.cumsum(-2)
g_A = g_A_cumsum.detach().clone().contiguous().requires_grad_(True)

g_B = torch.randn(batch, num_head, num_chunk, chunk_size, d_head, device='cuda').uniform_(0.8, 0.99).log()
