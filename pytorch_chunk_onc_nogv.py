...
from intra_chunk_loop import IntraChunkFusedLoop
...
def compute_inter(query,  decay_key,  memory_cache):
    return ((query * decay_key.exp()) @ memory_cache)    
...
class Chunk_memory_update(torch.autograd.Function):
    @staticmethod
    def forward(ctx, decay_key_last, to_add, use_triton=False):
    ...
    def backward(ctx, DO):
        ...
        D_p1 = torch.empty(B, H, N, D_v // BLOCK_MODEL, D_k, device=DO.device, dtype=decay_key_last.dtype)
        # D_p2 = torch.empty(B, H, N, D_k // BLOCK_MODEL, D_v, device=DO.device, dtype=decay_value_last.dtype)
        ...
        return D_p1.sum(-2), output, None         
        

@torch.jit.script
def prepare_to_add(g_key, key, value):
    ...
    to_add = (key * reduce_chunk_key).transpose(-1, -2) @ (value)


    return to_add, g_key_cumsum


    
    
def torch_chunk_parallel_onc_nogv(
    key, value,
    g_key,  
    query, chunk_size=16, use_triton=True, use_cuda=True, use_fused_loop=False  
) -> torch.Tensor:
    '''
    query, query: bf16
    '''
    ...
    to_add, g_key_cumsum = prepare_to_add(g_key,  key, value)


    decay_key_last = g_key_cumsum[..., -1, :].exp()    
    # decay_value_last = g_value_cumsum[..., -1, :].exp()

    memory_cache = Chunk_memory_update.apply(decay_key_last, to_add, True)    
    
    inter_chunk_contribution = compute_inter(query, g_key_cumsum,  memory_cache)
    
    inner_chunk_contribution = cuda_compute_intra_chunk64x_d64x(query, key, g_key_cumsum) 
    
    inner_chunk_contribution = inner_chunk_contribution @ value
