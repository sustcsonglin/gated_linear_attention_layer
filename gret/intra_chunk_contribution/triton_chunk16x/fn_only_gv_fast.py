...
def _fwd_compute_O(
    ...
    for q_high in range(lo+16, hi, 16):
        ...
        for k_high in range(0, q_high, 16):            
            ...
            #bf16
            output = tl.dot(qk, v, allow_tf32=False) * tl.exp(q_gv_normalizer - k_gv_normalizer).to(v.dtype)[None, :]     
            acc += output
...
class FlashGRet_O_Faster(torch.autograd.Function):
    @staticmethod
...
def compute_inner_o(qk, value, decay_value):
    ...
    decay_value_last = decay_value[..., -1, None, :]
    
    value = value *  (decay_value_last - decay_value).exp()
    return ((qk @ value) * (decay_value - decay_value_last).exp()).to(original_dtype)
...
if __name__ == "__main__":
    ...
    for _ in range(200):
        ...
        if requires_grad:
            o.sum().backward(retain_graph=True)    
        o2 = FlashGRet_O_Faster.apply(A, v, gk3)
        if requires_grad:
            o2.sum().backward(retain_graph=True)    
    ...
    for _ in range(200):
        o2 = FlashGRet_O_Faster.apply(A, v, gk)
        if requires_grad:
            o2.sum().backward(retain_graph=True)    
